...
const container = { hidden: {}, visible: { transition: { staggerChildren: 0.12, delayChildren: 0.12 } } };
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        {[...Array(20)].map((_, i) => (
          <motion.div
            ...
            style={{
              ...
              top: `${Math.random() * 100}%`,
              background: 'linear-gradient(90deg, rgba(0,240,255,0.3), rgba(192,132,252,0.3))'
            }}
            animate={{
              opacity: [0.2, 0.8, 0.2],
              scale: [0.5, 1, 0.5],
        ...
        <div className="absolute inset-0 bg-gradient-radial from-purple-500/5 via-transparent to-transparent dark:from-purple-600/10" />
      </div>
        ...
        >
          {features.map((f, idx) => (
            <motion.article
              ...
              whileHover={{
                scale: 1.03,
                boxShadow: '0 25px 50px -12px rgba(0, 240, 255, 0.3)',
              }}
              transition={{ type: 'spring', stiffness: 200, damping: 20 }}
              className="relative group"
            >
              <div className="h-full p-8 rounded-2xl backdrop-blur-xl transition-colors duration-300
                              bg-white/10 dark:bg-white/5 border border-black/10 dark:border-white/10">
                <motion.div
                  animate={{ y: [0, -8, 0] }}
                  transition={{
                    repeat: Infinity,
                  ...
                  className="h-full"
                >
                  <h4 className="text-lg md:text-xl font-semibold mb-4 flex items-center gap-3 text-black dark:text-white group-hover:text-cyan-300 transition-colors duration-300">
                  ...
                  <p className="text-base text-black/60 dark:text-white/60 group-hover:text-black/80 dark:group-hover:text-white/80 transition-colors duration-300">
                    {f.text}
                  </p>
                </motion.div>
